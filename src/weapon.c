...
void initWeapon(Weapon* self, int birthTextureId, int deathTextureId,
    ...
    *self = (Weapon){WEAPON_SWORD_POINT,
                     32 * 2,
                     40,
                     10,
                     60,
...
void initWeapons() {
    ...
    initWeapon(now = &weapons[WEAPON_THUNDER], RES_BLOOD_BOUND, RES_Thunder,
               -1);
    now->wp = WEAPON_SWORD_RANGE;
    now->damage = 80;
    now->shootRange = 128;
    ...
    initWeapon(now = &weapons[WEAPON_THUNDER_STAFF], -1, RES_THUNDER_YELLOW,
               -1);
    now->wp = WEAPON_SWORD_RANGE;
    now->damage = 50;
    now->shootRange = 128;
    ...
    initWeapon(now = &weapons[WEAPON_ARROW], -1, RES_HALO_EXPLOSION2,
               RES_ARROW);
