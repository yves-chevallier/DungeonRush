...
void initCountDownBar() {
    createAndPushAnimation(&animationsList[RENDER_LIST_UI_ID],
                           &textures[RES_SLIDER], NULL, LOOP_INFI, 1,
                           SCREEN_WIDTH / 2 - 128, 10, SDL_FLIP_NONE, 0,
                           AT_TOP_LEFT);
    countDownBar = createAndPushAnimation(&animationsList[RENDER_LIST_UI_ID],
                                          &textures[RES_BAR_BLUE], NULL,
                                          LOOP_INFI, 1, SCREEN_WIDTH / 2 - 128,
                                          10, SDL_FLIP_NONE, 0, AT_TOP_LEFT);
}
void initInfo() {
    extern int stage;
    char buf[1 << 8];
    ...
    if (self->bp == BLOCK_TRAP) {
        ...
        if (self->enable && self->ani->origin != &textures[RES_FLOOR_EXIT]) {
            ...
            createAndPushAnimation(
                ...
                &textures[RES_FLOOR_EXIT], &effects[EFFECT_BLINK], LOOP_INFI,
                30, self->x, self->y, SDL_FLIP_NONE, 0, AT_TOP_LEFT);
        }
    }
}
...
void renderAnimation(Animation* ani) {
    ...
    SDL_RenderCopyEx(renderer, ani->origin->origin,
                     &(ani->origin->crops[stage]), &dst, ani->angle, &poi,
                     ani->flip);
...
void renderId() {
    ...
    for (int i = 0; i < playersCount; i++) {
        ...
        if (snake->sprites->head) {
            ...
            if (i == powerful)
                renderCenteredTextBackground(&texts[4 + i], snakeHead->x,
                                             snakeHead->y, 0.5);
...
void renderCountDown() {
    double percent = (double)(renderFrames % GAME_MAP_RELOAD_PERIOD) /
