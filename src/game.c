...
void appendSpriteToSnake(
    ...
    if (snake->sprites->head) {
        ...
        int delta =
            (snakeHead->ani->origin->width * SCALE_FACTOR +
...
bool takeWeapon(Snake* snake, Item* weaponItem) {
    ...
    for (LinkNode* p = snake->sprites->head; p; p = p->nxt) {
        Sprite* sprite = p->element;
        if (sprite->ani->origin ==
                commonSprites[weaponItem->belong].ani->origin &&
            sprite->weapon == commonSprites[weaponItem->belong].weapon) {
            sprite->weapon = weapon;
            Animation* ani = createAndPushAnimation(
                &animationsList[RENDER_LIST_EFFECT_ID], weaponItem->ani->origin,
                NULL, LOOP_INFI, 3, sprite->x, sprite->y, SDL_FLIP_NONE, 0,
                AT_BOTTOM_CENTER);
            bindAnimationToSprite(ani, sprite, true);
            sprite->hp +=
                GAME_HP_MEDICINE_EXTRA_DELTA / 100.0 * sprite->totalHp * 5;
...
void initEnemies(int enemiesCount) {
    ...
    for (int bossCount = 0; bossCount < bossSetting; bossCount++) {
        Point pos = getAvaliablePos();
        generateEnemy(pos.x, pos.y, 1, 1, SPRITE_BIG_ZOMBIE, SPRITE_BIG_DEMON,
                      1);
    }
}
...
void slowDownSnake(Snake* snake, int duration) {
    ...
    for (LinkNode* p = snake->sprites->head; p; p = p->nxt) {
        Sprite* sprite = p->element;
        Animation* ani = createAndPushAnimation(
            &animationsList[RENDER_LIST_EFFECT_ID], &textures[RES_SOLIDFX],
            effect, LOOP_LIFESPAN, 40, sprite->x, sprite->y, SDL_FLIP_NONE, 0,
            AT_BOTTOM_CENTER);
...
void shieldSprite(Sprite* sprite, int duration) {
    Animation* ani = createAndPushAnimation(
        &animationsList[RENDER_LIST_EFFECT_ID], &textures[RES_HOLY_SHIELD],
        NULL, LOOP_LIFESPAN, 40, sprite->x, sprite->y, SDL_FLIP_NONE, 0,
        AT_BOTTOM_CENTER);
    bindAnimationToSprite(ani, sprite, true);
    ani->lifeSpan = duration;
}
...
bool crushVerdict(Sprite* sprite, bool loose, bool useAnimationBox) {
    ...
    if (inr(x / UNIT, 0, n - 1) && inr(y / UNIT, 0, m - 1))
        ...
        return true;
    // Loop over the cells nearby the sprite to know better if it falls out of
    // map
    for (int dx = -1; dx <= 1; dx++)
        for (int dy = -1; dy <= 1; dy++) {
    ...
    for (int i = 0; i < spritesCount; i++) {
        ...
        for (LinkNode* p = spriteSnake[i]->sprites->head; p; p = p->nxt) {
            ...
            if (other != sprite) {
                SDL_Rect otherBox = useAnimationBox
                                        ? getSpriteAnimationBox(other)
                                        : getSpriteFeetBox(other);
                if (RectRectCross(&box, &otherBox)) {
                    if ((self && loose) ||
                        (p->pre && p->pre->element == sprite))
                        ;
                    else
                        return true;
                }
    ...
    for (int i = 0; i < SCREEN_WIDTH / UNIT; i++)
        for (int j = 0; j < SCREEN_HEIGHT / UNIT; j++)
            if (hasMap[i][j]) {
                ...
                if (isPlayer(snake)) {
                    SDL_Rect headBox =
                        getSpriteFeetBox(snake->sprites->head->element);
                    if (itemMap[i][j].type != ITEM_NONE) {
                        if (RectRectCross(&headBox, &block)) {
                            ...
                            if (itemMap[i][j].type == ITEM_HERO) {
                                ...
                                appendSpriteToSnake(snake, itemMap[i][j].id, 0,
                                                    0, RIGHT);
                                herosCount--;
                                removeAnimationFromLinkList(
                                    &animationsList[RENDER_LIST_SPRITE_ID],
                                    ani);
                                ...
                                takeHpMedcine(snake, GAME_HP_MEDICINE_DELTA,
                                              itemMap[i][j].type ==
                                                  ITEM_HP_EXTRA_MEDCINE);
                                flasksCount -=
                                    itemMap[i][j].type == ITEM_HP_MEDCINE;

                                removeAnimationFromLinkList(
                                    &animationsList[RENDER_LIST_MAP_ITEMS_ID],
                                    ani);
    ...
    for (LinkNode* p = snake->sprites->head; p; p = p->nxt) {
        ...
        if (sprite->hp <= 0) {
            ...
            createAndPushAnimation(
                &animationsList[RENDER_LIST_DEATH_ID], &textures[RES_SKULL],
                NULL, LOOP_INFI, 1,
                sprite->x +
                    randInt(-MAP_SKULL_SPILL_RANGE, MAP_SKULL_SPILL_RANGE),
                sprite->y +
                    randInt(-MAP_SKULL_SPILL_RANGE, MAP_SKULL_SPILL_RANGE),
                sprite->face == LEFT ? SDL_FLIP_NONE : SDL_FLIP_HORIZONTAL, 0,
                AT_BOTTOM_CENTER);
            createAndPushAnimation(
                &animationsList[RENDER_LIST_DEATH_ID], death,
                &effects[EFFECT_DEATH], LOOP_ONCE, SPRITE_ANIMATION_DURATION,
                sprite->x, sprite->y,
        ...
        &textures[randInt(RES_BLOOD1, RES_BLOOD4)],NULL , LOOP_INFI,
        SPRITE_ANIMATION_DURATION, sprite->x + randInt(-MAP_BLOOD_SPILL_RANGE,
        MAP_BLOOD_SPILL_RANGE), sprite->y + randInt(-MAP_BLOOD_SPILL_RANGE,
        MAP_BLOOD_SPILL_RANGE), sprite->face == RIGHT ? SDL_FLIP_NONE :
                ...
                for (LinkNode* p = spriteSnake[i]->sprites->head; p;
                    ...
                    if (RectRectCross(&box, &bulletBox)) {
                        ...
                        if (weapon->wp == WEAPON_GUN_POINT ||
                            ...
                            invokeWeaponBuff(bullet->owner, weapon,
                                             spriteSnake[i], weapon->damage);
                            return hit;
                        }
                        break;
                    }
...
void moveSnake(Snake* snake) {
    ...
    for (LinkNode* p = snake->sprites->head; p; p = p->nxt) {
        ...
        for (int i = 0; i < step; i++) {
            ...
            while (b->size && sprite->x == firstSlot.x &&
                   sprite->y == firstSlot.y) {
        ...
        for (int j = 0; j < SCREEN_HEIGHT / UNIT; j++) {
            if (hasMap[i][j] && map[i][j].bp == BLOCK_TRAP) {
                if (!maskedTime)
                    createAndPushAnimation(
                        &animationsList[RENDER_LIST_MAP_SPECIAL_ID],
                        &textures[RES_FLOOR_SPIKE_OUT_ANI], NULL, LOOP_ONCE,
                        SPIKE_ANI_DURATION, i * UNIT, j * UNIT, SDL_FLIP_NONE,
                        0, AT_TOP_LEFT);
                else if (maskedTime == SPIKE_ANI_DURATION - 1) {
                    map[i][j].enable = true;
                    map[i][j].ani->origin = &textures[RES_FLOOR_SPIKE_ENABLED];
                } else if (maskedTime ==
                           SPIKE_ANI_DURATION + SPIKE_OUT_INTERVAL - 1) {
                    createAndPushAnimation(
                        &animationsList[RENDER_LIST_MAP_SPECIAL_ID],
                        &textures[RES_FLOOR_SPIKE_IN_ANI], NULL, LOOP_ONCE,
                        SPIKE_ANI_DURATION, i * UNIT, j * UNIT, SDL_FLIP_NONE,
                        0, AT_TOP_LEFT);
        ...
        if (snake->team != spriteSnake[i]->team) {
            for (LinkNode* p = spriteSnake[i]->sprites->head; p; p = p->nxt) {
                ...
                if (distance((Point){sprite->x, sprite->y},
                    ...
                    continue;
                double rad =
                    atan2(target->y - sprite->y, target->x - sprite->x);
                if (weapon->wp == WEAPON_SWORD_POINT ||
                    ...
                    invokeWeaponBuff(snake, weapon, spriteSnake[i],
                                     weapon->damage);
                    attacked = true;
                    if (weapon->wp == WEAPON_SWORD_POINT) goto ATTACK_END;
                } else {
                    Bullet* bullet =
                        createBullet(snake, weapon, sprite->x, sprite->y, rad,
                                     snake->team, weapon->flyAni);
                ...
                if (player->playerType == LOCAL) {
                    if (!player->buffs[BUFF_FROZEN] &&
                        ...
                        if (direction >= 0) {
                            ...
                            changeSpriteDirection(player->sprites->head,
                                                  direction);
                        }
                    }
                }
