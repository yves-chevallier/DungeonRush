...
const double MAP_WALL_HOW_DECORATED =
    0.1;  // this will take effect in pushMaptoRender in game.c
...
void initBlock(Block* self, BlockType bp, int x, int y, int bid, bool enable) {
    ...
    if (bp == BLOCK_TRAP) {
        self->ani = createAnimation(
            &textures[enable ? RES_FLOOR_SPIKE_ENABLED
                             : RES_FLOOR_SPIKE_DISABLED],
            NULL, LOOP_INFI, 1, x, y, SDL_FLIP_NONE, 0, AT_TOP_LEFT);
    } else if (bp == BLOCK_EXIT) {
        self->ani = createAnimation(
            &textures[enable ? RES_FLOOR_EXIT : RES_FLOOR_2], NULL, LOOP_INFI,
            1, x, y, SDL_FLIP_NONE, 0, AT_TOP_LEFT);
    } else {
        self->ani = createAnimation(&textures[bid], NULL, LOOP_INFI, 1, x, y,
        ...
        for (int j = 0; j < m; j++) {
            if ((i != exitX || j != exitY) && hasMap[i][j]) {
                if (isTrap[i][j])
                    ...
                    initBlock(&map[i][j], BLOCK_FLOOR, i * UNIT, j * UNIT,
                              RES_FLOOR_1, true);
            }
        }
}
...
void initBlankMap(int w, int h) {
    ...
    for (int i = 0; i < w; i++)
        for (int j = 0; j < h; j++) {
            int ii = si + i, jj = sj + j;
            hasMap[ii][jj] = 1;
            initBlock(&map[ii][jj], BLOCK_FLOOR, ii * UNIT, jj * UNIT,
                      RES_FLOOR_1, false);
        }
}
            ...
            if (!hasMap[i][j]) {
                if (inr(j + 1, 0, m - 1) && hasMap[i][j + 1]) {
                    if (inr(i + 1, 0, n - 1) && hasMap[i + 1][j]) {
                        createAndPushAnimation(
                            ...
                            LOOP_INFI, 1, i * UNIT, j * UNIT, SDL_FLIP_NONE, 0,
                            AT_TOP_LEFT);
                        createAndPushAnimation(
                            &animationsList[RENDER_LIST_MAP_ID],
                            &textures[RES_WALL_CORNER_BOTTOM_RIGHT], NULL,
                        ...
                        createAndPushAnimation(
                            &animationsList[RENDER_LIST_MAP_ID], &textures[bid],
                            NULL, LOOP_INFI, 1, i * UNIT, j * UNIT,
                            SDL_FLIP_NONE, 0, AT_TOP_LEFT);
                        createAndPushAnimation(
                            &animationsList[RENDER_LIST_MAP_ID],
                            &textures[RES_WALL_TOP_MID], NULL, LOOP_INFI, 1,
                            i * UNIT, (j - 1) * UNIT, SDL_FLIP_NONE, 0,
                            AT_TOP_LEFT);
                    }
                }
                if (inr(j - 1, 0, m - 1) && hasMap[i][j - 1]) {
                    ...
                    createAndPushAnimation(&animationsList[RENDER_LIST_MAP_ID],
                                           &textures[bid], NULL, LOOP_INFI, 1,
                                           i * UNIT, j * UNIT, SDL_FLIP_NONE, 0,
                                           AT_TOP_LEFT);
                    if (hasMap[i - 1][j]) {
                        createAndPushAnimation(
                            &animationsList[RENDER_LIST_MAP_FOREWALL],
                ...
                if (inr(i + 1, 0, n - 1) && hasMap[i + 1][j]) {
                    if (inr(j + 1, 0, m - 1) && hasMap[i][j + 1])
                        ...
                        createAndPushAnimation(
                            &animationsList[RENDER_LIST_MAP_ID],
                            &textures[RES_WALL_SIDE_MID_LEFT], NULL, LOOP_INFI,
                            1, i * UNIT, j * UNIT, SDL_FLIP_NONE, 0,
                            AT_TOP_LEFT);
                    if (!hasMap[i + 1][j + 1])
                        createAndPushAnimation(
                            &animationsList[RENDER_LIST_MAP_ID],
                    ...
                    if (!hasMap[i + 1][j - 1]) {
                        createAndPushAnimation(
                            &animationsList[RENDER_LIST_MAP_ID],
                            &textures[RES_WALL_SIDE_MID_LEFT], NULL, LOOP_INFI,
                            1, i * UNIT, (j - 1) * UNIT, SDL_FLIP_NONE, 0,
                            AT_TOP_LEFT);
                        createAndPushAnimation(
                            &animationsList[RENDER_LIST_MAP_ID],
                            &textures[RES_WALL_SIDE_TOP_LEFT], NULL, LOOP_INFI,
                            1, i * UNIT, (j - 2) * UNIT, SDL_FLIP_NONE, 0,
                            AT_TOP_LEFT);
                    }
                }
                if (inr(i - 1, 0, n - 1) && hasMap[i - 1][j]) {
                    if (inr(j + 1, 0, m - 1) && hasMap[i][j + 1])
                        ...
                        createAndPushAnimation(
                            &animationsList[RENDER_LIST_MAP_ID],
                            &textures[RES_WALL_SIDE_MID_RIGHT], NULL, LOOP_INFI,
                            1, i * UNIT, j * UNIT, SDL_FLIP_NONE, 0,
                            AT_TOP_LEFT);
                    if (!hasMap[i - 1][j + 1])
                        createAndPushAnimation(
                    ...
                    if (!hasMap[i - 1][j - 1]) {
                        ...
                        createAndPushAnimation(
                            &animationsList[RENDER_LIST_MAP_ID],
                            &textures[RES_WALL_SIDE_TOP_RIGHT], NULL, LOOP_INFI,
                            1, i * UNIT, (j - 2) * UNIT, SDL_FLIP_NONE, 0,
                            AT_TOP_LEFT);
                    }
                }
